...
export function MarketCard({ market, isCompact = false, isLoading = false, className = '', onClick }: MarketCardProps) {
  ...
  const timeRemainingText = daysRemaining > 0 ? `${daysRemaining}d` : 'Ending';
  ...
  // Card wrapper based on interaction type
  ...
  return (
    <CardWrapper>
      <div className="terminal-card h-full overflow-hidden group hover:border-green-400/50 hover:shadow-lg hover:shadow-green-500/20 transition-all duration-300">
        {/* Terminal Header */}
        <div className="flex items-center justify-between p-3 border-b border-green-500/20">
          <div className="flex items-center space-x-2">
          ...
          <div className="grid grid-cols-2 gap-3 mb-3 text-xs font-mono">
            ...
            <div>
              ...
              <span className="text-purple-400 font-bold ml-1">
                {Math.floor((market.totalVolume ?? 0) / 100)}
              </span>
            </div>
          </div>
